/**
 * Those functions are from the lab.
 */
...
size_t buffer_find(buffer *buffer, const char *data, size_t data_size);

// finds data of size data_size in a buffer in a
// case-insensitive fashion and returns its position
size_t buffer_find_insensitive(buffer *buffer, const char *data, size_t data_size);
